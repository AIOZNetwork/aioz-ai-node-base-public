...
def make_output(output_obj: OutputObject, tmp_dir: Union[str, Path] = None):
    ...
    for k, v in output_obj:
        if isinstance(v, FileObject):
            ...
            if all(isinstance(elem, FileObject) for elem in v):
                key_files.add(k)
                output_dict[k] = [elem.write_buff(tmp_dir) for elem in v]
            elif all(isinstance(elem, dict) for elem in v):
                new_list = []
                for elem in v:
                    new_elem = {}
                    for subk, subv in elem.items():
                        if isinstance(subv, FileObject):
                            key_files.add(f"{k}[{subk}]")
                            new_elem[subk] = subv.write_buff(tmp_dir)
                        else:
                            new_elem[subk] = subv
